...
public class Forest
{
	...
	//Parses through grid and finds empty spaces
            ...
            for (int c = 0; c < SIZE; c ++)
	    {
                if (grid[r][c] == null)
		{
                    cellNum = r * SIZE + c; // convert to a one dimensional list
	...
	System.out.println(playerRow + playerColumn);	
        for(int r = playerRow - 1; r <= playerRow + 1; r++)
        {
            for(int c = playerColumn - 1; c <= playerColumn + 1; c++)
            {
                ...
                if(inBorders(r, c))
		{
		    if(grid[r][c]!=null && grid[r][c].getAppearance() == ForestItem.TOKEN_ICON)
		    {
			return true;
		    }
		}
	...
	//THE FOLLOWING ALGORITHM WAS ADAPTED FROM XIN LIU'S TUTORIAL CODE
        ...
        int y = playerRow + dy;
        if ( x < 0 || x > SIZE - 1 || y < 0 || y > SIZE - 1 || grid[y][x] !=  null && grid[y][x].getAppearance() != ForestItem.EXIT_ICON)
	{
            System.out.println();
	    System.out.println ("Either you're trying to punch a token or run into a wall,\nyou're not going anywhere");
	    System.out.println();
        }
	else
	{
