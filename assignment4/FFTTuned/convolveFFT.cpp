...
#define LOG_TWO         0.69314718                      // Code Tuning 3: Created constants at runtime for Log(2) to avoid lengthy calculations
#define LOG2ROUNDEDUP(n)                (n) = (log(n)/(LOG_TWO)) + 1; // Code Tuning 4: Inlined the LOG2 function
...
{
	int pow2 = input_size;
	pow2 = LOG2ROUNDEDUP(pow2) + 1; // Code Tuning 3 and 4
	pow2 = pow(2, pow2);
	int double_pow2 = pow2 + pow2;	// Code Tuning 2: Strength Reduction: Used addition instead of multiplication
	...
	//zero pad arrays
	// Code Tuning 5: Fusion
	for (int i = 0; i < double_pow2; i+=2)
	{
		// Code Tuning 1: Partial Unrolling
		x_complex[i] = 0.0;
		x_complex[i + 1] = 0.0;
		h_complex[i] = 0.0;
		h_complex[i + 1] = 0.0;
	...
	for (int i = 0; i < input_size; i++)
		x_complex[i + i] = input_signal[i];	// Code Tuning 2: Strength Reduction: Used addition instead of multiplication

	// Load data into h_complex to be used in fft
	for (int i = 0; i < impulse_size; i++)
		h_complex[i + i] = impulse_response[i];	// Code Tuning 2: Strength Reduction: Used addition instead of multiplication
	...
	{
		output_signal[i] = y_complex[i + i] / (double_pow2 + double_pow2);	// Code Tuning 2: Strength Reduction: Used addition instead of multiplication
	}
}
